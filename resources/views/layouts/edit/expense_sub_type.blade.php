...
@section('content')
  <div class="container container-fluid" style="margin-top:100px; width:70%">
    <div class="row">
      <div class="col-11">
        @if(isset($item))
          <h3>Editando Subtipo de Despesas</h3>
        @else
          <h3>Cadastro de Subtipo de Despesas</h3>
        @endif
      </div>
      <div class="">
        <a href="{{route('expense_sub_type.index')}}" class="btn btn-danger">
    ...
    <div class="pt-5">
      @if(isset($item))
        <form action="{{route('expense_sub_type.update',$item->id_expense_sub_type)}}" method="POST">
          @method('PUT')
      @else
        <form action="{{route('expense_sub_type.store')}}" method="POST">
      @endif  
        @csrf
